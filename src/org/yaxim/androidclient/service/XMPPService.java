...
public class XMPPService extends GenericService {
    ...
    private final RemoteCallbackList< IXMPPRosterCallback > mRosterCallbacks =
            new RemoteCallbackList< IXMPPRosterCallback >();
    ...
    public void syncContacts() {
            ...
            while ( cursor.moveToNext() ) {
                ...
                if ( hasPhoneNumber > 0 ) {
                    ...
                    while ( phoneCursor.moveToNext() ) {
                        String phoneNumber =
                                phoneCursor.getString( phoneCursor.getColumnIndex( NUMBER ) ).replaceAll( "\\s+", "" );
        ...
        final ContactSync sync = RoomsMessageFactory.getRoomsMessage( RoomsMessageType.ContactSync );
    ...
    private void createServiceChatStub() {
        ...
        mServiceChatConnection = new IXMPPChatService.Stub() {
            ...
            @Override
            public String sendFile( final String jabberID, final String fileName, final long size, final String key,
                    final String url, final String mimeType )
                    throws RemoteException {

                if ( mSmackable != null ) {
                    final FileMessage message = RoomsMessageFactory.getRoomsMessage( RoomsMessageType.File );
                    try {
            ...
            public String sendTaskResponse( final String selectedOption )
                ...
                if ( mSmackable != null ) {
                    final TaskResponse message = RoomsMessageFactory.getRoomsMessage( RoomsMessageType.TaskResponse );
            ...
            public String sendTask( final String roomID, final String text, final String recipient )
                ...
                if ( mSmackable != null ) {
                    final TaskMessage message = RoomsMessageFactory.getRoomsMessage( RoomsMessageType.Task );
                    try {
            ...
            public void inviteParticipant( final String roomID, final String jid ) throws RemoteException {
                ...
                if ( mSmackable != null ) {
                    final AddParticipantRequest message =
                            RoomsMessageFactory.getRoomsMessage( RoomsMessageType.AddParticipantRequest );
            ...
            public void kickParticipant( final String roomID, final String jid ) throws RemoteException {
                ...
                if ( mSmackable != null ) {
                    final KickParticipantRequest message =
                            RoomsMessageFactory.getRoomsMessage( RoomsMessageType.KickParticipantRequest );
                    message.setRoomID( roomID );
                    message.setParticipants( Arrays.asList( jid ) );
                    mSmackable.sendControlMessage( message );
                }
    ...
    private OpenRoomRequest createOpenRoomRequest( final String parentRoomID, final String topic,
            final String[] participants ) {

        String roomName = topic;
        if ( parentRoomID != null ) {
            final Cursor c =
                    getContentResolver().query( RosterProvider.ROOMS_URI, new String[] { RoomsConstants.NAME },
        ...
        mService2RosterConnection = new IXMPPRosterService.Stub() {
            ...
            public void sendRegistrationMessage1( final String phoneNumber )
                ...
                final RegistrationRequest request =
                        RoomsMessageFactory.getRoomsMessage( RoomsMessageType.RegistrationRequest );
            ...
            public void sendRegistrationMessage2( final String code, final String publicKey )
                ...
                final Registration registration = RoomsMessageFactory.getRoomsMessage( RoomsMessageType.Registration );
            ...
            public void searchContact( final String name ) throws RemoteException {
                ...
                final ContactSearch search = RoomsMessageFactory.getRoomsMessage( RoomsMessageType.ContactSearch );
            ...
            public void sendWebToken( final String token ) throws RemoteException {
                ...
                final Crypto crypto = YaximApplication.getApp( getApplicationContext() ).mCrypto;
                final WebLoginToken tokenMessage = RoomsMessageFactory.getRoomsMessage( RoomsMessageType.WebLoginToken );
                try {
            ...
            public void setCompanies( final String[] companyKeys ) throws RemoteException {
                ...
                final CompanyAffiliationResponse response =
                        RoomsMessageFactory.getRoomsMessage( RoomsMessageType.CompanyAffiliationResponse );
    ...
    private void updateServiceNotification() {
        ...
        final String title =
                getString( R.string.conn_title, sharedPreferences.getString( PreferenceConstants.NAME, "" ) );
    ...
    private void createAdapter() {
        ...
        mSmackable.registerCallback( new XMPPServiceCallback() {
            @Override
            public void newMessage( final String from, final String roomID, final String message,
                    final boolean silent_notification ) {

                logInfo( "notification: " + from );
                notifyClient( from, mSmackable.getNameForJID( from, roomID ), roomID, message, !mIsBoundTo
                        .contains( from )
                        && !mIsBoundTo.contains( roomID ), silent_notification, false );
            }
